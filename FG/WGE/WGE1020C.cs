...
namespace WG
{
    public partial class WGE1020C : CommonClass.FORMBASE
    {
        public WGE1020C()
        {
            InitializeComponent();
        ...
        protected override void p_SubFormInit()
        {
            ...
            p_ScIni(ss1, Sc1, 17, false, false);//垛位禁止排序
            iheadrow = 1;
            iscseq = 1;
            //0-5
           ...
           //6-10
            ...
            p_ScIni(ss2, Sc2, 17, false, false);//垛位禁止排序
            iheadrow = 1;
            iscseq = 2;
            //0-5
            ...
            //16-20
            p_SetSc("表面等级", "E", "10", "L", "", "", "", iscseq, iheadrow, "L");
            p_SetSc("作业人员", "E", "7", "IL", "", "", "", iscseq, iheadrow, "L");
            p_SetSc("库", "E", "2", "PILA", "PIAL", "", "", iscseq, iheadrow, "L");
            iheadrow = 0;
            p_spanSpread("规格", 7, 10, iscseq, iheadrow, 1);
            p_spanSpread("作业指示/实绩", 11, 15, iscseq, iheadrow, 1);
            //钢板号冻结
            ...
            if (ss1_Sheet1.RowCount > 0 && ss2_Sheet1.RowCount > 0 && e.RowHeader.ToString() != "删除")
            {
                ...
                else
                {
                    ...
                    //{
                        ...
                        if (ss1.ActiveSheet.Cells[e.Row, iss1_plate_no].Text.Trim() != "" || ss1.ActiveSheet.RowHeader.Rows[e.Row].Label != "删除")//将钢板移动到空的垛层
                        {
                            sxt_select_cnt.NumValue = 1;                        
                            //break;
                        }
                        //iPlatecnt++;
                    //}
        ...
        private void move_mark(int startrow)
        {
            for (int icnt =0 ; icnt < (int)sxt_select_cnt.NumValue; icnt++)
            {
                ...
                ss2.ActiveSheet.Cells[startrow - icnt, iss2_upd_emp].Text = GeneralCommon.sUserID;//记录指定垛位人员
