...
namespace CG
{
    public partial class WGF1010C : CommonClass.FORMBASE
    {
        public WGF1010C()
        {
            InitializeComponent();
        ...
        protected override void p_SubFormInit()
        {
            ...
            p_McIni(Mc1, true);
            imcseq = 1;
            p_SetMc("轧辊号",CMB_ROLL_NO, "PNIR", "", "", "","", imcseq);
            p_SetMc("工厂",TXT_PLT, "NIR", "", "", "","", imcseq);
            p_SetMc(CMB_SHIFT, "NI", "", "", "", imcseq, "");
            p_SetMc("班别",CMB_GROUP, "NI", "", "", "","", imcseq);
     ...
     //       base.sAuthority = "1111";
        ...
        {
            //检查班次输入的有效性
            if (CMB_SHIFT.Text != "1" && CMB_SHIFT.Text != "2" && CMB_SHIFT.Text != "3")
            {
                GeneralCommon.Gp_MsgBoxDisplay("请输入有效的班次", "", "");
                return;
            }
            if (CMB_ROLL_NO.Text.Substring(0, 1) != "R")
            {
                GeneralCommon.Gp_MsgBoxDisplay("轧辊号首位应为R,请正确输入轧辊号！", "", "");
                return;
            }
            p_Ref(1, 0, true, true);
            CMB_ROLL_NO.Enabled = true;
