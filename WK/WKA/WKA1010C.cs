...
namespace WK
{
    public partial class WKA1010C : CommonClass.FORMBASE
    {
        public WKA1010C()
        {
            InitializeComponent();
        ...
        protected override void p_SubFormInit()
        {
            ...
            p_McIni(Mc1, false);
            imcseq = 1;
            p_SetMc(txt_slab_edt_seq, "PR", "", "", "", imcseq, "");  // 轧制序号
            p_SetMc(txt_slab_from, "R", "", "", "", imcseq, "");      // 起始板坯号
            p_SetMc(txt_slab_to, "R", "", "", "", imcseq, "");        // 终止板坯号
            p_SetMc(txt_slab_target, "R", "", "", "", imcseq, "");    // 目标板坯号
            ...
            for (int lRow = 0; lRow <= ss1.ActiveSheet.RowCount - 1; lRow++)
            {
                if (ss1.ActiveSheet.Cells[lRow, PRC_STS_Color].Text == "B")
                {
                    if(ss1.ActiveSheet.Cells[lRow,iSs1_ORD_KND].Text == "实际订单")
                    {
                        ...
                        {
                            SpreadCommon.Gp_Sp_RowColor(ss1, lRow, Color.Red, label6.BackColor);
                        }
                ...
                else
                {
                    if (ss1.ActiveSheet.Cells[lRow, L2_SEND_Color].Text == "Y")
                    {
                        if (ss1.ActiveSheet.Cells[lRow, iSs1_ORD_KND].Text == "实际订单")
                        {
                                ...
                                if (ss1.ActiveSheet.Cells[lRow, iSs1_DEL_DATE].Text != ""&&int.Parse(ss1.ActiveSheet.Cells[lRow, iSs1_DEL_DATE].Text.Replace("-", "")) < XX)
                                {
                                    SpreadCommon.Gp_Sp_RowColor(ss1, lRow, Color.Red, label5.BackColor);
                                }

                        }
                        else
                        {
                            SpreadCommon.Gp_Sp_RowColor(ss1, lRow, Color.Black, label5.BackColor);
                        }
                        SpreadCommon.Gp_Sp_RowColor(ss1, lRow, Color.Black, label5.BackColor);
                    }
                }
        ...
        public override void Form_Pro()
        {
            ...
            if (rdo_send.Checked)
            {
                ...
                if (txt_slab_to.Text != "")
                {
                    if (GeneralCommon.Gf_MessConfirm("确定要下达到 '" + txt_slab_to.Text + "' 的作业指示吗？", "Q", "指示下达确定"))
                    {
            ...
            if (rdo_cancel.Checked)
            {
                ...
                if (txt_slab_from.Text != "")
                {
                    if (GeneralCommon.Gf_MessConfirm("确定取消从 '" + txt_slab_from.Text + "' 的作业指示吗？", "Q", "指示取消确定"))
                    {
            ...
            if (rdo_move.Checked)
            {
                ...
                Mode = "M";
                string sMsg = "";
                sMsg = "确定要把板坯从(" + txt_slab_from.Text + ")->(" + txt_slab_to.Text + ")" + "调整到板坯(" + txt_slab_target.Text + ")后边吗？ ";

                if (txt_slab_from.Text != "")
                {

                    if (txt_slab_from.Text != "" && txt_slab_to.Text != "" && txt_slab_target.Text != "")
                    {
            ...
            if (rdo_delete.Checked)
            {
                ...
                if (txt_slab_to.Text != "")
                {
                    if (GeneralCommon.Gf_MessConfirm("确定要删除选定板坯(" + txt_slab_from.Text + ")->(" + txt_slab_to.Text + ")吗？", "Q", ""))
                    {
            ...
            if (rdo_return.Checked)
            {
                ...
                if (GeneralCommon.Gf_MessConfirm("确定要返送选定板坯(" + txt_slab_from.Text + ")->(" + txt_slab_to.Text + ")吗？", "Q", ""))
                {
                    sRtn_Msg = Gp_Process_Exec(Mode);
                    if (sRtn_Msg == "")
                    {
        ...
        private void rdo_send_Click(object sender, EventArgs e)
        {
            if (rdo_send.Checked == true)
            {
                ...
                rdo_move.ForeColor = System.Drawing.Color.Black;
                rdo_delete.ForeColor = System.Drawing.Color.Black;
                rdo_return.ForeColor = System.Drawing.Color.Black;
        ...
        {
            if (rdo_move.Checked == true)
            {
                rdo_move.ForeColor = System.Drawing.Color.Red;
                rdo_send.ForeColor = System.Drawing.Color.Black;
                rdo_cancel.ForeColor = System.Drawing.Color.Black;
                rdo_delete.ForeColor = System.Drawing.Color.Black;
                rdo_return.ForeColor = System.Drawing.Color.Black;
        ...
        {
            if (rdo_return.Checked == true)
            {
                rdo_return.ForeColor = System.Drawing.Color.Red;
                rdo_send.ForeColor = System.Drawing.Color.Black;
                rdo_cancel.ForeColor = System.Drawing.Color.Black;
                rdo_move.ForeColor = System.Drawing.Color.Black;
                rdo_delete.ForeColor = System.Drawing.Color.Black;
        ...
        {
            if (txt_slab_to.Text.Trim() != "")
            {
                if (GeneralCommon.Gf_MessConfirm("确定要强制下达到 '" + txt_slab_to.Text + "' 的作业指示吗？", "Q", "指示下达确定"))
                {
        ...
        {
            if (txt_slab_target.Text.Trim() != "")
            {
                if (GeneralCommon.Gf_MessConfirm("确定从板坯（ '" + txt_slab_to.Text + "'）进行辊期编制吗？", "Q", "辊期编制确定"))
                {
        ...
        protected override void ss_CellClickEvent(object sender, CellClickEventArgs e)
        {
            ...
            if (rdo_from.Checked)
            {
                txt_slab_from.Text = ss1.ActiveSheet.Cells[e.Row, iSs1_Slab_No].Text;
                sSlab_Edt_Seq_Fr = ss1.ActiveSheet.Cells[e.Row, iSs1_Slab_Edt_Seq].Text;
            }

            if (rdo_to.Checked)
            {
                txt_slab_to.Text = ss1.ActiveSheet.Cells[e.Row, iSs1_Slab_No].Text;
                sSlab_Edt_Seq_To = ss1.ActiveSheet.Cells[e.Row, iSs1_Slab_Edt_Seq].Text;
            }

            if (rdo_target.Checked)
            {
                txt_slab_target.Text = ss1.ActiveSheet.Cells[e.Row, iSs1_Slab_No].Text;
            ...
            if (e.Column == iSs1_Slab_No)
            {
                ...
                txt_slab_edt_seq.Text = ss1.ActiveSheet.Cells[e.Row, iSs1_Slab_Edt_Seq].Text;

                if ((txt_slab_edt_seq.Text).Trim().Length > 0)
                {
